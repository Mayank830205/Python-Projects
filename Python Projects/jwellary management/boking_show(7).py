...
def show_record():
    ...
    conn,cur=db_connect()
    data = cur.execute("SELECT Full_Name, Mobile_No, Jwelery_type, Address, Booking_Time, Checkout_Date FROM booking")
    ...
    treeview=ttk.Treeview(columns=("Full_Name", "Mobile_No", "Jwelery_type","Address","Booking_Time","Checkout_Date"),show='headings',height=22)
    column=["Full_Name", "Mobile_No", "Jwelery_type","Address","Booking_Time","Checkout_Date"] # yaha pr column name change kar dena dono jagh upar aur niche
    for i in range(len(column)):

        treeview.column(f"# {i+1}",width=100,anchor=CENTER)
        treeview.heading(column[i],text=rec[i])
    for j in range(0,len(r_set)):
        treeview.insert('','end',values=r_set[j])
    treeview.place(x=22,y=170)
...
files1=Image.open("C:\\Users\\HP\\Desktop\\JWELLARY MANAGEMENT\\image\\img1.png")  # used for open image
files2=files1.resize((600,466)) #for assign size
