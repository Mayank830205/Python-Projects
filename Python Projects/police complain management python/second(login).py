...
def login(): #here we call function on login button who used to save data when we click
    ...
    if data: # if username data is present then
        ...
        if data:
            ...
            os.system("python fifth(home_page).py")                      # then open menubar page
...
def signup():
    os.system("python third(Signup).py") 
    
def reset():    
    os.system("python fourth(reset).py") 
...
win.config(bg="BLACK")
files1=Image.open("C:\\Users\\HP\\Desktop\\police complain management python\\image\\signup.png")  # used for open image
files2=files1.resize((1200,700)) #for assign size
